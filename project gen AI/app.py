...
def run_riasec_score_calculation():
    try:
        ...
        # Load user responses from the answers.txt file
        ...
        print("\nRIASEC Scores:")
        total_score = sum(int(response) for response in user_responses.values())
        user_scores = {}

        for (question, category), response in zip(questions, user_responses.values()):
            category = category.split()[0]  # Extract the category abbreviation (e.g., "Realistic (R)" => "Realistic")
            user_scores[category] = int(response)
            print(f"{category}: {response}")

        # Normalize the scores to a total of 100
        normalized_scores = {category: (score / total_score) * 100 for category, score in user_scores.items()}

        print("\nNormalized RIASEC Scores (out of 100):")
        for category, score in normalized_scores.items():
            print(f"{category}: {score:.2f}%")

        # Write the normalized RIASEC scores to riasec_results.txt
...
@app.route('/aptitude-test', methods=['POST'])
def aptitude_test():
    try:
        data = request.get_json()
